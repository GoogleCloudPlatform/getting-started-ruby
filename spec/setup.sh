...
if [ "$STEP_NAME" = '2-cloud-datastore' -o "$STEP_NAME" = 'optional-kubernetes-engine' ]; then
  ...
  cloud-datastore-emulator/gcd.sh create -d gcd-test-dataset-directory gcd-test-dataset-directory
  cloud-datastore-emulator/gcd.sh start --testing ./gcd-test-dataset-directory/ &
fi
